...
# other levels
cordata$pair_traits <- forcats::fct_cross(cordata$typeNEO, cordata$traitPID)
cordata$pair_traits <- forcats::fct_relevel(cordata$pair_traits, "Openness:Antagonism")
cordata$pair_facets <- forcats::fct_cross(cordata$typeNEO, cordata$typePID)
cordata$pair_facets <- forcats::fct_relevel(cordata$pair_facets, "Openness:deceitfulness")
...
```{r, message = FALSE}
ggplot(data = left_join(cordata, corstats, by = c("typeNEO", "traitPID")), 
       aes(x=meddist, y=corrs)) + 
  geom_boxplot(aes(group = pair_traits), alpha = 0.5, staplewidth = 60) + 
...
```{r, message = FALSE}
fit <- glm(abs(corrs) ~ cosdist + pair_traits, family = quasibinomial, data = cordata)
cf <- summary(fit)$coefficients
cf["cosdist",]

# plot
cf <- coef(fit)
ggplot(cordata, aes(x=cosdist, y=fitted(fit))) + 
...
Once the existence of an association between semantic distance and response correlations has been demonstrated, the important question remains of estimating response correlations at the net of the correlation predicted by semantic distance. We would like to model correlations as determined by two components: one produced by semantic similarity, and one that represents candidate genuine correlations.

The approach that we will follow here is to model square correlations rather than the correlations themselves. This has two reasons: one is that semantic similarity predicts positive as well as negative correlations, without predicting which of the two; the second, that square correlations appear more suited to model additive components to the observed association. The drawback is that square correlations are biased away from zero, although this problem can be alleviated by including a bias correction in the computations.

To obtain these measures of association, one may adjust for the semantic distance between item pairs. We need to assume that the adjusted squared correlations are homogeneous in terms of the sign of the correlation, otherwise the larger squared correlation may be a misleading indicator of the correlations, which may be made null by the opposite directions. For this reason, we use the PID facets instead of the PID traits.

We model the NEO subscale/PID facet as a random effect. In this model,

semantic distance is the effect of semnantic similarity,
constant + facet is the explained variance for each facet/NEO scale pair
...
fitlo <- stan_lmer(log(abs(corrs^2) / (1 - abs(corrs^2))) ~ cosdist.b + (1 | pair_facets), data = mutate(cordata, cosdist.b = cosdist - 0.75), cores = 4)

plotdata <- as.matrix(fitlo)
plotdata <- plotdata[,1] + plotdata[,3:(ncol(plotdata)-2)]
mcmc_intervals(exp(plotdata)/(1+exp(plotdata))) + geom_vline(xintercept = 0) + theme_minimal()
rm(plotdata)
...
Papke, Leslie E and Wooldridge, Jeffrey M (1996). "Econometric methods for fractional response variables with an application to 401(k) plan participation rates". Journal of Applied Econometrics 11:619-632.  (the fractional logistic model approach, also called quasi-binomial model, check McCullagh & Nelder 1989)
...
Ferrari, S., & Cribari-Neto, F. (2004). Beta regression for modelling rates and proportions. Journal of applied statistics, 31(7), 799-815.  (the original formulation of beta regression)

Rigby, R. A., & Stasinopoulos, D. M. (2005). Generalized additive models for location, scale and shape. Journal of the Royal Statistical Society Series C: Applied Statistics, 54(3), 507-554. (allows for flexibility in handling various types of response variables, including bounded and ordinal data)

Cribari-Neto, F., & Zeileis, A. (2010). Beta Regression in R. Journal of Statistical Software, 34(2), 1–24. https://doi.org/10.18637/jss.v034.i02 (a good explanation, no, an excellent explanation)

Ali, Gabry, & Goodrich. Modelling rates/proportions using beta regression with rstanarm. 2020. https://mc-stan.org/rstanarm/articles/betareg.html
