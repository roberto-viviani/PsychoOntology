...
To obtain these measures of association, one may adjust for the semantic distance between item pairs. A grouping variable pools an overall estimate of correlation (and relative confidence intervals) between subscale pairs. That is, we use the fitted correlation estimate as a proxy of the adjusted correlation.

To estimate this quantity statistical and provide inference, we may use the inference for the subscale-pair intercepts. In a fixed effects model, this quantity is relative to a reference subscale-pair. Using a random effect to estimate these group effects has the advantage that the estimates are relative to a general mean effect.

A further improvement of the model is to allow the slopes to vary in each subscale pair. This model allows the relationship between semantic similarity and response correlations to vary, because there are factors that may influence this relationship between the items, such as varying amount of genuine correlations. Because a model with fixed effects here gives the impression of overfitting, we use random effects. We log-transform the rates, as if they were odds, to fit a linear model. We use stan to get good estimate of intervals.
...
fitlo <- stan_lmer(log(abs(corrs^2) / (1 - abs(corrs^2))) ~ pair + cosdist.b + (cosdist.b - 1 | pair), data = cordataex, cores = 4)

plotdata <- as.matrix(fitlo)
plotdata <- plotdata[,2:31]
...
ggplot(cordataex, aes(x = cosdist, y = exp(fitted)/(1+exp(fitted)))) + 
