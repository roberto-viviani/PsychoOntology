...
pearscorr <- function(data, yvar, xvars) {
  stats <- data.frame()
  for (v in xvars) {
    cs <- cor.test(scales[,yvar], scales[,v], 
              method = "pearson", alternative = "two.sided")
    stats[v,"r"] <- cs$estimate
    stats[v,"t"] <- cs$statistic
    stats[v,"p"] <- cs$p.value
    stats[v, "pcorr"] <- ifelse(cs$p.value < 0.001 / length(xvars), "**", 
                                ifelse(cs$p.value < 0.01 / length(xvars), "*", "-"))
  }
  stats
}
...
print(pearscorr(scales, "NEOneuro", pidtraits))
...
p <- baseplot(plotdata, "Neuroticism")
...
ggsave("neuroticism_openAI_large.pdf",  width = 150, height = 110, units = "mm")
...
stats <- pearscorr(scales, "NEOneuro", c("PIDNegativeAffect", "PIDDetachment"))
...
print(pearscorr(scales, "NEOextra", pidtraits))
...
p <- baseplot(plotdata, "Extraversion")
p <- addneoline(p, plotdata, "Detachment")
p <- addneoline(p, plotdata, "Disinhibition")
p <- addneoline(p, plotdata, "Antagonism")
p + geom_text(data = filter(plotdata, (cosdist < 0.5 & corrs < -0.2) | cosdist < 0.45), 
            aes(label = typePID), alpha = 0.4) 

ggsave("extraversion_openAI_large.pdf", width = 150, height = 110, units = "mm")
...
print(pearscorr(scales, "NEOgew", pidtraits))
...
p <- baseplot(plotdata, "Conscientiousness")
p <- addneoline(p, plotdata, "Anancasticity")
p <- addneoline(p, plotdata, "Disinhibition")
p + geom_text(data = filter(plotdata, cosdist < 0.45), 
            aes(label = typePID), alpha = 0.4) 

#ggsave("conscientiousness_openAI_large.pdf", width = 150, height = 110, units = "mm")
...
#ggsave("conscientiousness_detail_openAI_large.pdf", width = 150, height = 60, units = "mm")
...
#ggsave("conscientiousness_detail_gecko.pdf", width = 150, height = 60, units = "mm")
...
#ggsave("conscientiousness_detail_openAI_large_en.pdf", width = 150, height = 60, units = "mm")
...
#ggsave("conscientiousness_detail_gecko_en.pdf", width = 150, height = 60, units = "mm")
...
print(pearscorr(scales, "NEOvertr", pidtraits))
...
p <- baseplot(plotdata, "Agreeableness")
p <- addneoline(p, plotdata, "Antagonism")
...
ggsave("agreeableness_openAI_large.pdf", width = 150, height = 110, units = "mm")
...
print(pearscorr(scales, "NEOoff", pidtraits))

```

```{r openness, message = FALSE}
plotdata <- filter(cordata, typeNEO == "Openness")
p <- baseplot(plotdata, "Openness")
p <- addneoline(p, plotdata, "Detachment")
p <- addneoline(p, plotdata, "NegativeAffect")
p <- addneoline(p, plotdata, "Psychoticism")
...
ggsave("openness_openAI_large.pdf", width = 150, height = 110, units = "mm")
