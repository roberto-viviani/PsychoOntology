...
if os.path.exists('embeddings_mistral_en.csv'):
...
if 'item_en' in data.columns:
    data = data.rename(columns={'item_en': 'item'})
    data = data.drop(columns=['item_de'])
...
data.to_csv('embeddings_mistral_en.csv', index=False)
