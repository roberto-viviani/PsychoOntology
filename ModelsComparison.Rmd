...
We test here the extent to which semantic similarity predicts the respondent correlation of answers across subscales using different large language models.
...
scales <- read.csv("scales_data_extended.csv") |> select(-NEOFFI61:-NEOFFI66)
...
```{r, message = FALSE, warning = FALSE}
#Cohere for clustering and for classification are very similar
embeddings <- readembeds("embeddings_cohere_clust_de.csv")
NEO <- filter(embeddings, scaleID == "NEO") |> select(itemID, type, embedding)
PID <- filter(embeddings, scaleID == "PID") |> select(itemID, type, embedding)
cordata  <- getcordata(crcosdist(embmx(NEO), embmx(PID)))
...
plotfunc(cordata)
#ggsave("subscales_alephalpha_base_128.pdf",         
#       plot = plotfunc(cordata) + guides("color" = "none", "shape" = "none"),
#       width = 150 * 2 / 3, height = 95 * 3 / 4, units = "mm")
testfunc(cordata)
rm(list = c("embeddings", "NEO", "PID", "cordata"))
...
Like Roberta, this is a freely available model. Nothing to gain here.
