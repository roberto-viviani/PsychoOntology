...
scales <- read.csv("scales_data.csv") |> tidyr::drop_na() |> select(-NEOFFI61:-NEOFFI66)
...
# model and inference. Cosine distance multiplied by 10 to have coefs for 0.1 cosine
testfunc <- function(data) {
  summary(glm(abs(corr) ~ I(dist * 10), 
...
Roberta performs well, although it is not clear if it supports the German language.

```{r, message = FALSE, warning = FALSE}
embeddings <- readembeds("embeddings_roberta_de.csv")
NEO <- filter(embeddings, scaleID == "NEO") |> select(itemID, type, embedding)
PID <- filter(embeddings, scaleID == "PID") |> select(itemID, type, embedding)
cordata  <- getcordata(crcosdist(embmx(NEO), embmx(PID)))

plotfunc(cordata)
ggsave("subscales_roberta_small.pdf", 
        plot = plotfunc(cordata) + guides("color" = "none", "shape" = "none"),
        width = 150 * 2 / 3, height = 95 * 3 / 4, units = "mm")
testfunc(cordata)
rm(list = c("embeddings", "NEO", "PID", "cordata"))
...
We look at whether performance can be improved by combining the information from models. We do not use mpnet because it performs poorly, but include universal sentence encoder as it may provide different information.
...
NEO <- filter(embeddings, scaleID == "NEO") |> select(itemID, type, embedding)
PID <- filter(embeddings, scaleID == "PID") |> select(itemID, type, embedding)
distmx_aleph <- crcosdist(embmx(NEO), embmx(PID))

distmx_pooled <- (distmx_openAI + distmx_gecko + distmx_aleph) / 3
...
This reveals that the average model performed just better than the best model. However, the advantage accruing from pooling the models is small.

We add all models to the pool, to verify robustness to model selection.
...
distmx_pooled <- (distmx_openAI + distmx_gecko + distmx_roberta + 
                    distmx_USE + distmx_mistral + distmx_alephalpha) / 5

cordata_pooled <- getcordata(distmx_pooled)
plotfunc(cordata_pooled)
testfunc(cordata_pooled)
rm(list = c("embeddings", "NEO", "PID", "distmx_openAI", "distmx_gecko", "distmx_alephalpha",
     "distmx_roberta", "dismx_USE", "distmx_mistral", "distmx_pooled", "cordata_pooled"))
```
